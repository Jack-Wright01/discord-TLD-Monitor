...
async def setMessageAction(bool, message=None):
    ...
    await log.discord(body=f"Successfully set the bot to monitor URLs in this server, messages with unapproved Top Level Domains will **not** be deleted", footer=f"See `{PREFIX}help action` for more info", channel=DEFAULT_CHANNEL)
